 ...
 */
class WizardryInnMainUi extends WizardryCastleUi {
  /**
  ...
  activateCharacter(index) {
    ...
    let action = () => {
      ...
      PARENT_OBJECT._parent.room = WizardryConstants.INN_ROOM_NONE;
      PARENT_OBJECT._parent.state = WizardryConstants.INN_PLAYER_MENU;
    };
    if (character.status !== WizardryCharacterStatus.OK) {
      ...
      action = () => {
        ...
        PARENT_OBJECT._messageBlock.text = "A rest won't help them. Try the Temple.";
  ...
  init() {
    ...
    this._parent.createScreenOutline({
      name: [WizardryConstants.INN_MAIN, "_ui_frame"].join(""),
      cols: 40,
      rows: 24,
    ...
    grid.addControl(this._parent.createTextBlock({ text: "WELCOME TO THE ADVENTURER'S INN" }), 0, 0);
    ...
    { // entry prompt and cursor
      ...
      panel.addControl(this._parent.createTextBlock({ text: "WHO WILL STAY ? >", horizontalAlignment: BABYLON.GUI.Control.HORIZONTAL_ALIGNMENT_LEFT }));
