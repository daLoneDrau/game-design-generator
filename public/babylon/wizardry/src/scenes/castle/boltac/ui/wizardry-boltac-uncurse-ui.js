 ...
 */
class WizardryBoltacUncurseUi extends WizardryCastleUi {
  /**
    ...
    switch (key.toUpperCase()) {
      ...
      case "8":
        ...
        if (this._displayRows[index].isVisible) {
          this.uncurse(this._displayRows[index].name);
        } else {
          validInput = false;
        }
        break;
  ...
  init() {
    ...
    this._parent.createScreenOutline({
      name: [WizardryConstants.BOLTAC_UNCURSE_MENU, "_ui_frame"].join(""),
      cols: 40,
      rows: 24,
    ...
    { // entry prompt and cursor
      ...
      panel.addControl(this._parent.createTextBlock({ text: "WHICH DO YOU WISH UNCURSED ? >", horizontalAlignment: BABYLON.GUI.Control.HORIZONTAL_ALIGNMENT_LEFT }));
  ...
  set() {
    ...
    if (typeof(character) !== "undefined") {
      for (let i = 0, li = this._displayRows.length, j = 0; i < li; i++) {
        if (character.possessions.possession[i].equipmentIndex >= 0 && character.possessions.possession[i].cursed) {
  ...
  /**
   * Attempts to uncurse an item. Uncursing costs 1/2 the item's price and destroys it.
   * @param {String} index the index of the item being uncursed
   */
  uncurse(index) {
    const character = WizardryController.rosterInstance.getCharacterRecord(WizardryController.characterRecord);
    ...
    if (!possessionObject.cursed) {
      // uncursed items can't be "uncursed". set a timer to clear the user entry prompt
      PARENT_OBJECT.set();
      ...
      if (character.gold < price) {
        ...
        // take the player's gold and destroy the item
...
export { WizardryBoltacUncurseUi };