...
const SHOP_MESSAGES = [
  ...
  "** YOU CANT AFFORD THE FEE **",
  "** ANYTHING ELSE, SIRE? **"
];
/**
 * @class Ui class for the Identify Menu state of the Shop scene.
 */
class WizardryBoltacIdentifyUi extends WizardryCastleUi {
  /**
  ...
  handleKeyEntry(key) {
    ...
    switch (key.toUpperCase()) {
      ...
      case "8":
        ...
        if (this._displayRows[index].isVisible) {
          this.identify(this._displayRows[index].name);
        } else {
          validInput = false;
        }
        break;
  ...
  /**
   * Attempts to identify an item. Identification costs 1/2 the item's price.
   * @param {String} index the index of the item being identified
   */
  identify(index) {
    ...
    if (possessionObject.identified) {
      // identified items can't be "re-identified". set a timer to clear the user entry prompt
      PARENT_OBJECT.set();
      ...
      if (character.gold < price) {
        // player can't afford to identify
        PARENT_OBJECT.set();
        PARENT_OBJECT._parent.stopAnimation(PARENT_OBJECT._messageBlock);
        ...
        // take the player's gold and identify the item
  ...
  init() {
    ...
    this._parent.createScreenOutline({
      name: [WizardryConstants.BOLTAC_IDENTIFY_MENU, "_ui_frame"].join(""),
      cols: 40,
      rows: 24,
    ...
    { // display player's inventory
      ...
      for (let i = 0; i < 8; i++) {
        ...
        this._displayRows.push(clickableRow);
      }
    }
    { // entry prompt and cursor
      ...
      panel.addControl(this._parent.createTextBlock({ text: "WHICH DO YOU WISH IDENTIFIED ? >", horizontalAlignment: BABYLON.GUI.Control.HORIZONTAL_ALIGNMENT_LEFT }));
  ...
  prepForExit() {
    ...
    for (let i = this._displayRows.length - 1; i >= 0; i--) {
      this._displayRows[i].resetHighlights();
    }
  }
  /**
  ...
  set() {
    ...
    for (let i = this._displayRows.length - 1; i >= 0; i--) {
      let widget = this._displayRows[i];
      widget.name = "";
    ...
    if (typeof(character) !== "undefined") {
      for (let i = 0, li = this._displayRows.length, j = 0; i < li; i++) {
        if (character.possessions.possession[i].equipmentIndex >= 0 && !character.possessions.possession[i].identified) {
          const possessionObject = character.possessions.possession[i];
          const item = WizardryController.equipmentListInstance.getEquipmentItem(possessionObject.equipmentIndex);
          let widget = this._displayRows[j++];
...
export { WizardryBoltacIdentifyUi };