...
import { WizardryAttribute,
  WizardryConstants,
  WizardryRace }                  from "../../../../config/wizardry-constants.js";
import { WizardryCharacterMaker } from "../../../../services/wizardry-character-maker.js";

/**
 * @class Ui class for the Race entry state of the Make Character scene.
 */
class WizardryMakeCharacterRaceUi extends WizardryUiConfiguration {
  /**
  ...
  /**
   * Chooses a character's race.
   * @param {string} letter the letter of the selection
   * @param {WizardryRace} race the race chosen
   */
  chooseRace(letter, race) {
    if (!this._inWait) {
      this._inWait = true;
      this._entryBlock.text = [letter, race.title].join(") ");
      WizardryCharacterMaker.setRace(race);
  ...
  handleKeyEntry(key) {
    if (!this._inWait) {
      ...
      switch (key.toUpperCase()) {
        case "A":
          this.chooseRace(key.toUpperCase(), WizardryRace.HUMAN);
          break;
        case "B":
          this.chooseRace(key.toUpperCase(), WizardryRace.ELF);
          break;
        case "C":
          this.chooseRace(key.toUpperCase(), WizardryRace.DWARF);
          break;
        case "D":
          this.chooseRace(key.toUpperCase(), WizardryRace.GNOME);
          break;
        case "E":
          this.chooseRace(key.toUpperCase(), WizardryRace.HOBBIT);
          break;
        case "ESCAPE":
          this.prevScreen();
          break;
  ...
  init() {
    this._parent.createScreenOutline({
      name: [WizardryConstants.MAKE_CHARACTER_RACE, "_ui_frame"].join(""),
      cols: 40,
      rows: 24,
    ...
    { // race options/messages
      ...
      { // options and messages
        ...
        { // options
          ...
          const races = WizardryRace.values;
          for (let i = 1, li = races.length; i < li; i++) {
            optionsGrid.addControl(this._parent.createButton({
              background: {
                horizontalAlignment: BABYLON.GUI.Control.HORIZONTAL_ALIGNMENT_LEFT,
                onPointerClickObservable: [() => { this.chooseRace(WizardryConstants.ALPHABET[i - 1], races[i]); }],
                onPointerEnterObservable: [
                  () => {
                    // stop the fade animation and restore the alpha on the message block
                    this._messageBlock.alpha = 1;
                    this._parent.stopAnimation(this._messageBlock);
                    this._messageBlock.text = races[i].description;
                  }
                ],
                onPointerOutObservable: [
                  () => {
                    // clear the tooltip if it matches the selected tooltip text
                    if (this._messageBlock.text === races[i].description) {
                      this._messageBlock.text = "";
                    }
                  }
                ]
              },
              text: { text: [WizardryConstants.ALPHABET[i - 1], ") ", races[i].title].join("") }
            }).container, i - 1, 0);
          }
        }
        { // message block
  ...
  prevScreen() {
    if (!this._inWait) {
      this._parent.state = WizardryConstants.MAKE_CHARACTER_NAME;
    }
  }
  /**
...
export { WizardryMakeCharacterRaceUi };